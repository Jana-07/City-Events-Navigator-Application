...
class UserRepository {
  ...
  Future<void> removeFavorite(String userId, String favoriteId) async {
    ...
    await _firestoreService.deleteDocument('users/$userId/favorites/$favoriteId');
  }
  ...
  Future<bool> isEventFavorite(String userId, String eventId) async {
    ...
    final snapshot = await _firestoreService.getCollection(
      'users/$userId/favorites',
      queryBuilder: (query) => query.where('eventRef', isEqualTo: FirebaseFirestore.instance.doc('events/$eventId')),
      limit: 1,
    );
